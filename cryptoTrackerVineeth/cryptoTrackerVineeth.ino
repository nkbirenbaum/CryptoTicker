...
char urlPrice[] = "https://api.coingecko.com/api/v3/simple/price?ids=cardano&vs_currencies=usd&include_market_cap=true&include_24hr_vol=true&include_24hr_change=true&include_last_updated_at=true";
char urlPing[] = "https://api.coingecko.com/api/v3/ping";
char coinName[] = "cardano";
...
void fetchPrice() {
  ...
  coin.price = doc["cardano"]["usd"];
  coin.percentChange = doc["cardano"]["usd_24h_change"];
  ...
  snprintf(coin.priceBuffer1, Coin::bufferSize, "ADA: $%0.2f          ", coin.price);
  ...
  time_t utcTimestamp = doc["cardano"]["last_updated_at"];
...
void updateClock() {
  ...
  char currentTime[16];
  if ( gmtOffsetSec == 3600) {
    snprintf(currentTime, 16, "%s:%s %s", hour12StrPlusll, minStr, ampm);
  } else {
